...
export class ExportController {
  ...
  async egeFromExcel(@UploadedFile() file: Express.Multer.File) {
    ...
    for(let x in response) {
      const ID = abits.find((abit) => response[x].passport == `${abit.passport_series} ${abit.passport_num}` && response[x].fio.toLowerCase() == `${abit.lastName.toLowerCase()} ${abit.firstName.toLowerCase()} ${abit.surName.toLowerCase()}`)?abits.find((abit) => response[x].fio.toLowerCase() == `${abit.lastName.toLowerCase()} ${abit.firstName.toLowerCase()} ${abit.surName.toLowerCase()}`).id : null
  ...
  async callsFromExcel(@UploadedFile() file: Express.Multer.File) {
    const abits = await this.abitService.find()
    const fioAbits = abits.map(abit =>{
      return {fio:`${abit.lastName.toLowerCase()} ${abit.firstName.toLowerCase()} ${abit.surName.toLowerCase()}`,ld:abit.personal_file_number}
    })
    const response = await this.exportService.callsFromExcel(file)
    const data = []
    response.map(info => {
      const inf = {ld:null,fio:null,date:null, num:null, date_admission:null, test:null}
      inf.ld = info.ld
      inf.date = info.date
      inf.num = info.num
  ...
  async egeToCsv(@Res() res: Response){
    ...
    for (let i of abits){
      if (i.passport_num){
        info=info+`${i.lastName};${i.firstName};${i.surName};${i.passport_series};${i.passport_num}\n`
      }
    }
    const data = {fileName:path.resolve(__dirname, '..', '..', '..', '..', 'EXPORT_FILES','egeCSV.csv'),data:info}
